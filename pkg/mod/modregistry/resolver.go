...
func (r *resolver) Fetch(ctx context.Context, m module.Version) (module.SourceLoc, error) {
	if f, ok := r.resolved.Load(m); ok {
		return f.(module.SourceLoc), nil
	}

	s, err := r.Resolve(ctx, m.Path(), m.Version())
	if err != nil {
		return module.SourceLoc{}, errors.Errorf("fetch go mod failed %s", err)
	}

	r.resolved.Store(m, s)
...
func (r *resolver) Resolve(ctx context.Context, mpath string, version string) (module.SourceLoc, error) {
	...
	return r.convertToCueMod(ctx, mpath, info)
}
