...
import (
	...
	"github.com/octohelm/cuekit/internal/gomod/internal/cmd/go/internals/cache"
	...
	"github.com/octohelm/cuekit/internal/gomod/internal/cmd/go/internals/par"
...
func (rp *IndexPackage) Import(bctxt build.Context, mode build.ImportMode) (p *build.Package, err error) {
	...
	if !inTestdata(rp.dir) {
		...
		// GOROOT or GOPATH. Since module mode only calls Type with path set to "."
		...
		if ctxt.GOROOT != "" && str.HasFilePathPrefix(p.Dir, cfg.GOROOTsrc) && p.Dir != cfg.GOROOTsrc {
			...
			// are only set in build.Type if p.Root != "".
