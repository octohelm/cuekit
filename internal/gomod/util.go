...
func Get(ctx context.Context, path string, version string, verbose bool) *Module {
	...
	found, err := modload.ListModules(ctx, []string{requestPath}, modload.ListRetractedVersions, "")
	if err != nil {
...
func ListVersion(ctx context.Context, path string, reuseFile string) ([]string, error) {
	found, err := modload.ListModules(ctx, []string{path}, modload.ListVersions, reuseFile)
	if err != nil {
		return nil, err
	}
	if len(found) > 0 {
